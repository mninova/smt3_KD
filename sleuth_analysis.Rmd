...
```{r, sleuth_wald_FC,  message=FALSE, fig.height=5, fig.width=8}
...
res<-dplyr::left_join(res,table.srbp, by="target_id") %>% mutate(signFC=(qval<0.05 & abs(log2FC)>1), significant=qval<0.05)  %>% mutate(signFC2=ifelse(qval<0.05 & abs(log2FC)>2, 2, ifelse(qval<0.05 & abs(log2FC)>1, 1, 0))) %>% arrange(qval)

p<-ggplot(res, aes(mean_obs, b)) + theme_classic() +  ggtitle("MA plot of genes and TEs, sleuth") +
 geom_point(aes(colour = as.factor(signFC2), shape = TE), alpha = 0.5) + 
  scale_colour_manual(values = c("black", "orange","red")) + scale_shape_manual(values = c(15,16)) + 
    xlab(paste("mean( log(counts + 0.5 ) )")) + 
      ylab(paste0("beta: conditionKD")) 

p1<-p + geom_text(data=(res %>% filter(str_detect(target_id,"(smt3|^R1_DM|^R2_DM)"))), aes(mean_obs, b, label=sub("_DM.*","",target_id)), position = position_nudge(x=0.7))

p2<-res %>% mutate("type"=as.factor(ifelse(str_detect(TE, "TRUE"),"TE","RefSeq"))) %>% ggplot(aes(y=log2FC, x=type)) + geom_boxplot() + theme_light() + ggtitle("log2FC by type") + scale_y_continuous(breaks=seq(-5,10,1))

gridExtra::grid.arrange(p1,p2, widths=c(4,1))
